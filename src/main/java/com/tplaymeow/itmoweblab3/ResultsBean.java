...
public class ResultsBean {
    ...
    @lombok.Generated
    private static final java.util.logging.Logger log = java.util.logging.Logger.getLogger(ResultsBean.class.getName());
    ...
    public void setDatabase(DataSource dataSource) {
        if (Objects.nonNull(dataSource)) repository = new ResultsRepositoryImpl(dataSource);
    }

    public void calculate() {
        if (Objects.isNull(repository)) return;
        Result result = new Result(coordinates.getX(), coordinates.getY(), coordinates.getR(), checker.check(coordinates), LocalDateTime.now());
...
//        PrimeFaces.current().executeScript("createDot('" + result.getX() +"', '" + result.getY() +"', '" + result.getSuccess() + "')");
    ...
    public List<Result> getResults() {
        if (Objects.isNull(repository)) return new ArrayList<>();
